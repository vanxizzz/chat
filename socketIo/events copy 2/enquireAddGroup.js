...
async function enquireAddGroup({ targetId, groupData = {}, newGroup, allow }, socket, io) {
    // if (!targetData) {
    ...
    let { id: groupId, name: groupName, avatar: groupAvatar } = groupData;
    /* 是新群的话先创建 */
    if (newGroup) {
...
exports.event = (socket, io) => {
    socket.on("enquireAddGroup", async ({ targetId, groupData, newGroup = false, allow = null }) => {
        enquireAddGroup({ targetId, groupData, newGroup, allow }, socket, io);
    })
}